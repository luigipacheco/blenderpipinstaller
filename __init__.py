...
class PIP_OT_uninstall_library(Operator):
    bl_idname = "pip.uninstall_library"
    bl_label = "Uninstall Library"
    bl_description = "Uninstall the specified Python library"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        props.installing = True
        props.last_result = f"Uninstalling {library_name}..."
        ...
        def uninstall_thread():
            try:
                ...
                result = subprocess.run([python_exe, "-m", "pip", "uninstall", "-y", library_name],
                                      capture_output=True, text=True, timeout=120)
                
                if result.returncode == 0:
                    props.last_result = f"✓ Successfully uninstalled {library_name}"
                    print(f"Successfully uninstalled {library_name}")
                    props.library_installed = False
                    props.installed_version = ""
                else:
                    error_msg = result.stderr.strip() if result.stderr else "Unknown error"
                    props.last_result = f"✗ Failed to uninstall {library_name}: {error_msg}"
                    print(f"Failed to uninstall {library_name}: {error_msg}")
                    
            except subprocess.TimeoutExpired:
                props.last_result = f"✗ Uninstallation of {library_name} timed out"
                print(f"Uninstallation of {library_name} timed out")
            except Exception as e:
                props.last_result = f"✗ Error uninstalling {library_name}: {str(e)}"
                print(f"Error uninstalling {library_name}: {str(e)}")
            finally:
                props.installing = False
    ...
    def execute(self, context):
        ...
        def upgrade_thread():
            try:
                ...
                props.last_result = "✗ Pip upgrade timed out"
    ...
    def draw(self, context):
        ...
        if props.library_name.strip() and props.last_result and not props.installing:
            if props.library_installed:
                ...
                uninstall_op = row.operator("pip.uninstall_library", text="Uninstall", icon='TRASH')
